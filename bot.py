...
async def setup_container(container_id, status_msg, memory, username, vps_id=None, use_custom_image=False):
    ...
    try:
        ...
        hostname_cmd = f"echo 'Lemon Nodes-{vps_id}' > /etc/hostname && hostname LemonNodes-{vps_id}"
...
async def on_ready():
    ...
    try:
        await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="Lemon Nodes VPS"))
...
@app_commands.describe(
    ...
    use_custom_image="Use custom Nissal Nodes image (recommended)"
)
async def create_vps_command(ctx, memory: int, cpu: int, disk: int, owner: discord.Member, 
    ...
    try:
        ...
        status_msg = await ctx.send("🚀 Creating Lemon Nodes VPS instance... This may take a few minutes.")
        ...
        if use_custom_image:
            await status_msg.edit(content="🔨 Building Pterodactyl Docker image...")
            try:
